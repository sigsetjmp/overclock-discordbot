...
// key of the actual rendered content; identical key => no need to edit Discord
function contentKey(snapshot) {
  if (!snapshot || !Array.isArray(snapshot.players)) return JSON.stringify({ f: false });
  const rows = [...snapshot.players]
    .sort((a, b) => a.name.localeCompare(b.name))
    .map((p) => [p.name, p.kills, p.deaths, p.damage, p.heal]);
  return JSON.stringify({ f: true, p: rows, m: snapshot.mvp ?? null });
}
...
class LiveBoard {
  ...
  async handle(snapshot) {
    ...
    try {
      ...
      const key = contentKey({ players, mvp });
